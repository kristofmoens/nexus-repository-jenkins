    ...
    extends JenkinsITSupport
{
  ...
  public void retrieveJenkinsUpdateCenterFromProxyWhenRemoteOnline() throws Exception {
    ...
    String expected = new String(wantedContent).replace("{{ target url }}",nexusUrl.toString().replace("http","https") + "repository/jenkins-test-proxy/download");
    String resString = byteArrayOutputStream.toString();
    //assertEquals("The resulting string should have more or less the same amount of bytes", expected.length()/100, resString.length()/100);
    assertEquals("Update-center.json url content should have been changed correctly", expected.replace(",","\n,"), resString.replace(",","\n,"));
  ...
  public void retrieveApkArchiveFromProxyWhenRemoteOnline() throws Exception {
    ...
    assertTrue(componentAssetTestHelper.componentExists(proxyRepo, CRUNCH_SECURITY_PLUGINNAME + JENKINS_PLUGIN_EXTENSION, CRUNCH_SECURITY_VERSION));
    assertTrue(componentAssetTestHelper.componentExists(proxyRepo, ANCHORCHAIN_PLUGINNAME + JENKINS_PLUGIN_EXTENSION, ANCHORCHAIN_VERSION));
  }
